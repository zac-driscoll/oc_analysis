...
mds_params <-
  param_lookup_table %>%
  filter(op_sid %in%
           c(877, 196, 192, 189, 75, 213, 181,198,191)) %>% #TC, TP, NH3,  NO5,  TSS, Fecal, Temp,Chloride,TKN
  pull(op_sid)
...
calc_t_test <- function(site1,site2,param){
  ...
  if (param %in% c(877, 196, 192, 189, 183, 179,198)) {
    oc_data_t <- oc_data_t %>%
...
all_t_tests <-
  expand.grid(oc_sites, oc_sites, c(mds_params, 880, 178, 182, 179,183))
...
oak_wshed <- get_watershed_layer(watershed_code = "040400020102")
...
leaflet() %>%
  addPolygons(data = oak_wshed, color = "green",
              opacity = 0.5, 
              fillOpacity = 0.1) %>% 
...
<br>
<br>
<button class="btn btn-primary" data-toggle="collapse" data-target="#btn-ops-198"> Chloride </button> 
<div id="btn-ops-198" class="collapse"> 
```{r}

z <- make_table(198)

DT::datatable(make_table(198)) %>% 
  DT::formatStyle(
    colnames(z[2:8]),
    color = styleInterval(c(3.4, 3.8), c('white', 'blue', 'red')),
    backgroundColor = styleInterval(c(0.05), c('coral','dodgerblue'))
...
<br>
<br>
<button class="btn btn-primary" data-toggle="collapse" data-target="#btn-ops-191"> TKN </button> 
<div id="btn-ops-191" class="collapse"> 
```{r}

z <- make_table(191)

DT::datatable(make_table(191)) %>% 
  DT::formatStyle(
    colnames(z[2:8]),
    color = styleInterval(c(3.4, 3.8), c('white', 'blue', 'red')),
    backgroundColor = styleInterval(c(0.05), c('coral','dodgerblue'))
...
<br>
<br>
<button class="btn btn-primary" data-toggle="collapse" data-target="#btn-sum"> Summary </button> 
<div id="btn-sum" class="collapse"> 
```{r}

sum_t <- all_t_tests %>%
  rename(op_sid = ops) %>%
  left_join(param_lookup_table %>%
              select(op_sid,label_name)) %>%
  filter(site_1 %in% c("OC-03","OC-04","OC-06","OC-07") | 
           site_2 %in% c("OC-03","OC-04","OC-06","OC-07")) %>%
  filter(op_sid != 880) %>%
  mutate(different = if_else(p_value > 0.05, "yes","no")) %>%
  select(-op_sid,-different) %>%
  pivot_wider(names_from = c(site_1,site_2), values_from = p_value) %>%
  select(label_name, "OC-06_OC-07","OC-03_OC-04") 
...
Parameters used within the analysis include: TC, TP, NH3,  NO5,  TSS, Fecal, 
Temperature, Cl ,TKN.  Data were scaled by parameter groups using z scale 
...
<h3> MDS Plot </h3>
Points are colored by month <br>

```{r, echo = FALSE, fig.width = 11, fig.height = 7}

p <- oc_mds3 %>%
  ggplot(aes(x = V1, y = V2, color = month, 
             text = sitecode, label = month)) +
  geom_point(size = 4) +
  theme(legend.position = "right",
        legend.text=element_text(size=12),
